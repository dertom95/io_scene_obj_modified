...
def create_materials(filepath, relpath,
                     ...
                     use_image_search, float_func,map_to_existing_materials):
    """
    ...
    # list with names of materials that are already exist in blender and will be used instead of creating a new one
    mapped_material_names = []
    # Create new materials
    for name in unique_materials:  # .keys()
        ma_name = "Default OBJ" if name is None else name.decode('utf-8', "replace")
        
        ma = None
        if map_to_existing_materials and ma_name in bpy.data.materials:
            print("\tmapping to existing material %s",ma_name)
            unique_materials[name] = bpy.data.materials[ma_name]
            mapped_material_names.append(name)
            continue
        else:
    ...
    for libname in sorted(material_libs):
        ...
        if not os.path.exists(mtlpath):
            ...
            for line in mtl:  # .readlines():
                ...
                if skip_till_next_mtl and line_id != b'newmtl':
                    continue

                if line_id == b'newmtl':
                    # Finalize previous mat, if any.
                    if skip_till_next_mtl:
                    ...
                    print("NEW MATERIAL: %s" % context_material_name)

                    if context_material_name in mapped_material_names:
                        skip_till_next_mtl = True
                        continue
...
def load(context,
         ...
         *,
         ...
         relpath=None,
         global_matrix=None,
         map_to_existing_materials=False
         ):
    """
